...
/**
 * Request API to create a file then dispatch defined events
 * @param {String} createFileName
 * @returns {Function}
 */
export const createNewFile = (createFileName) => (dispatch, getState) => {
  const { path } = getState();
  dispatch(setLoading(true));

  APIHandler.createFile(path.join('/'), createFileName).then(r => {
